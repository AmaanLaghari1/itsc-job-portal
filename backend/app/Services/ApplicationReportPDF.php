...
class ApplicationReportPDF extends FPDF
{
...
//        $printHeader();
    ...
    public function extractQualificationValue($qualifications, $degreeTitle)
    {
        foreach ($qualifications as $qual) {
            if (
                isset($qual['discipline']['degree']['DEGREE_TITLE']) &&
                $qual['discipline']['degree']['DEGREE_TITLE'] == $degreeTitle
            ) {
                // Prefer GRADE > CGPA > marks
                if (str_contains($degreeTitle, 'MATRIC') || str_contains($degreeTitle, 'INTER')) {
                    return $qual['discipline']['DISCIPLINE_NAME'] . ": \n" . "Grade: ". $qual['GRADE'];
                }
                else {
                    $result = empty($qual['CGPA']) ? "Grade: ". $qual['GRADE'] : "CGPA: ". $qual['CGPA'];
                    return $qual['discipline']['DISCIPLINE_NAME'] . ": \n".$result;
                }
            }
        }

        return '-'; // if degree not found
    }

//    public function requiredData(){
//        $record = Announcement::with(['applications', 'applications.user', 'applications.user.district', 'applications.user.qualifications.discipline.degree'])->where('ANNOUNCEMENT_ID', 11)->get();;
//        return $record;
//    }

    public function requiredData($announcementIds)
    {
        return Announcement::whereIn('ANNOUNCEMENT_ID', $announcementIds)
...
//            })
            ->with([
                ...
                'applications.user',
                'applications.user.district',
                'applications.user.qualifications.discipline.degree'
            ])
            ->get();
