...
class ScrutinyReportPDF extends FPDF
{
...
//        $printHeader();
    ...
    public function requiredData($announcementId)
    {
        return $records = DB::table('applications_scrutiny as s')
            ->join('applications as a', 'a.APPLICATION_ID', '=', 's.APPLICATION_ID')
            ->join('announcements as an', 'an.ANNOUNCEMENT_ID', '=', 's.ANNOUNCEMENT_ID')
            ->join('application_status as st', 'st.APPLICATION_STATUS_ID', '=', 's.APPLICATION_STATUS_ID')
            ->select(
                's.ID',
            ...
            ->where('an.ANNOUNCEMENT_ID', $announcementId)
            ->get();
    }
